...
| 17c | `obj.required_count` | AT-9 consent: detecting that the threshold the attesters were **shown** differs from the one applied | **B** | Signature-covered field of the verified Attestation Object, compared against `quorum_k`. It is **not** an input to the quorum decision (that is row 17b, **R**) and cannot become one. A first pass at this fix classified it as consumed by *nothing* and deleted the comparison; see §4 T-33 for why that was wrong. |
...
**A second route to the same place, found while fixing the first.** The threshold was one way one key holder satisfied a quorum; the registry was another. `quorum_k` counts distinct `attester` **strings**, and the registry maps a string to a key — so two identities registered against one public key let the holder of that one private key sign two objects with different nonces, label them with the two names, and satisfy k=2 alone. Enforced as **PB-DISTINCT** at bundle construction, not on the quorum path: a registry that cannot support its own quorum is malformed for every reader of the bundle. It is not expressible in `attesters.schema.json` — JSON Schema has no uniqueness keyword for the values of a map — so the schema states the rule and names the loader as the enforcement point. Replayed as a live attack and mutation-proven (Suite 1 `PB-DISTINCT one key, two names`; Suite 2 `attester key distinctness [PB-DISTINCT]`). A matching bar on the receipt key appearing in the registry was considered and **not** added: at any k ≥ 2 it removes no compromise, and the property that actually matters — attesters are humans reading a rendered proposal (A-8) — is not checkable from key material.

*Disposition (fixed):* `Bundle.quorum_k` is a required, un-defaulted field inside `Bundle.hash()`; the threshold is recomputed from it (**R**); and every attestation's `required_count` must equal it (**B**). Normative as **PB-6** and **AT-9** in ACP-SPEC-001 v1.3.15.

**A correction inside the correction, and it is the more instructive half.** The first pass at this fix added the recomputation and *deleted* the equality check, arguing that `quorum_k` is inside the bundle hash and every entry's `policy_bundle_hash` is already checked against it, so the threshold is bound transitively and the equality check kills no mutant — a check that kills no mutant is not a control. Every step of that is true. The conclusion is still wrong, because the enumeration behind it contained only attacks that **lower** the threshold.

Raising the stated count is not an attack on INV-1-HIGH. It is an attack on **consent**, and it belongs to AT-3, not to the invariant: attesters shown "3 approvals required" sign on that basis, and executing after two removes a reviewer whose participation their signature assumed. Nothing is forged, no signature is invalid, the recomputed threshold is met, and the humans approved a different action than the one that happened. It has its own attack (`AT-9 signed for a larger quorum`) and its own mutant.

*The general form:* **"this check is redundant" is a claim about the attacks you enumerated, not about the check.** Both redundancy arguments made during this fix were locally valid and globally wrong. That is the same failure as the enumeration gap that produced T-33 in the first place, made twice more by the party fixing it, which is about as clear a demonstration as this document is going to get that the method's weak step is enumeration and not classification.
