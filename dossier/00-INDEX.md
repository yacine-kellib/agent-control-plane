...
**Specification version:** ACP-SPEC-001 v1.3.15
**Date:** August 2026
...
Two consequences a reader should know before verifying. The gate is **split**: `./tools/verify.sh --suites` runs the proofs, all 15 suites and the external-corpus harness without the release key and is green at every commit, while full `verify.sh` adds integrity and signature and is green only at a tagged release — sections 1–2 red between releases is a property of offline signing, not a finding. And **the services are no longer here**: all six moved to a separate product repository in the two-repository split, so nothing in this repository can be mistaken for a running control plane — there is nothing left to mistake, which is weaker than a scaffold that refuses to start and is disclosed as such in §06. `spec/vectors/` — the shared corpus that would make "52/52" mean the same in both languages — is not yet extracted. Six new residuals (RES-P1..P6) are disclosed in §06; the load-bearing one is that splitting the notifier and approval codebases does **not** close T-32.
