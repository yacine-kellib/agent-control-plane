...
- **CL-7.** Every claim operation (hit and miss) **MUST** be audited with identifier, class, timestamp, and Executor identity.
...
| **Consumption Ledger** | Permit replay / attestation reuse | **Violation possible — disclosed.** | TCB per §4.3. CL-1/CL-2 require linearizability and fail-closed; a ledger that falsely reports "absent" reopens T-09 and T-14. Mitigate by consensus-backed implementation and CL-7 auditing. |
...
- **AU-1.** Chains are per-tenant. `chain_hash_n = SHA-256(chain_hash_{n-1} ‖ canonical(record_n))`.
...
- **AU-8 (NEW in v1.3.10 — closes Y5.1). Chain genesis is specified.** `chain_hash_0` is the SHA-256 of the canonical encoding of a **tenant-creation record** `{tenant_id, created_at, bundle_epoch, schema_version}`, anchored **immediately** on tenant creation, not on the ≤ 10 min schedule. *Rationale:* without an anchored genesis, deletion of an entire tenant chain is detectable only after the first scheduled anchor, so a chain destroyed inside its first ten minutes leaves no evidence it existed.
