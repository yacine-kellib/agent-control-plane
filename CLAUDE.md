...
./tools/selftest.sh                # tests the tooling itself (121 assertions)
...
**`spec/schemas/` now has two roots.** `spec/schemas/bundle/` has seven schemas and is read by `tools/codegen.sh`; **`spec/schemas/wire/` is new (ACP-78) and is read by nothing that generates** — `tools/check-nonce-type.py` evaluates one field's pattern against the reference, and that is the whole of its executable consumption. Giving it a generator is slice 3, and it is sequenced before the remaining ~7 message schemas on purpose: the ACP-44 precedent is that *the generator reading the schema is what finds the defects*, so schemas written ahead of it land unchecked. The one that exists already earned that ordering — writing it surfaced ACP-87, a normative type the specification did not have. `spec/vectors/` has `CLASSIFICATION.md` and `OBLIGATIONS.md` — 86 cases split 48 vector-expressible / 38 per-implementation obligations. **Extracting the actual vector corpus has not started** (VEC-2), and it is what `crates/acp-conformance` waits on. Note the limit recorded there: a vector names a **seed plus the declared derivation**, never a seed alone — an implementation choosing its own domain separators derives a different identity and refuses every signature in the vector, which at the verifier is indistinguishable from a forgery.
