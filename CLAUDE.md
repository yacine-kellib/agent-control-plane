...
cd reference/suites
PYTHONPATH=../src python3 conformance.py          # 50/50
PYTHONPATH=../src python3 attack_registry.py      # 79/79  (--compose → 4/4)
PYTHONPATH=../src python3 mutate_executor.py      # 24/24  deletes each check, asserts the attack succeeds
...
`MANIFEST.sha256` goes stale the moment any covered file is edited. The release action is `./tools/sign-release.sh sign <keyfile>`, which only the key holder can run. Coverage is 128 files across ten roots.

Not yet started: `spec/schemas/` and `spec/vectors/` are empty. Extracting the vector corpus, and classifying which of the 50 conformance cases are data-expressible versus per-implementation obligations, is the next step and the thing `crates/acp-conformance` waits on.
