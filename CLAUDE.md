...
cd reference/suites
PYTHONPATH=../src python3 conformance.py          # 53/53
PYTHONPATH=../src python3 attack_registry.py      # 82/82  (--compose → 4/4)
PYTHONPATH=../src python3 mutate_executor.py      # 26/26  deletes each check, asserts the attack succeeds
...
cargo check --workspace && cargo test --workspace   # Rust: 164 tests
...
`MANIFEST.sha256` goes stale the moment any covered file is edited. The release action is `./tools/sign-release.sh sign <keyfile>`, which only the key holder can run. Coverage is 146 files across 8 roots.
