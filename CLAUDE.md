...
cd reference/suites
PYTHONPATH=../src python3 conformance.py          # 47/47
PYTHONPATH=../src python3 attack_registry.py      # 76/76  (--compose → 4/4)
PYTHONPATH=../src python3 mutate_executor.py      # 22/22  deletes each check, asserts the attack succeeds
...
Not yet started: `spec/schemas/` and `spec/vectors/` are empty. Extracting the vector corpus, and classifying which of the 47 conformance cases are data-expressible versus per-implementation obligations, is the next step and the thing `crates/acp-conformance` waits on.
